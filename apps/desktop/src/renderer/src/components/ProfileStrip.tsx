    ...
    <div className="profile-strip">
      {profiles.map((p) => (
        <button key={p.id} className={"profile-dot" + (p.id === active ? " active" : "")} title={p.name} aria-label={p.name}
          style={{ ["--dot" as string]: p.color }} onClick={() => run(() => selectProfile(p.id))}>
          <Icon name={p.icon} size={14} />
        </button>
      ))}
      {adding ? (
        <form className="inline-form" onSubmit={(e) => { e.preventDefault(); if (name.trim()) run(() => createProfile(name.trim())); setName(""); setAdding(false); }}>
