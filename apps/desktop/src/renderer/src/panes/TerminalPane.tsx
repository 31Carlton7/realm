 ...
 *
 *  Takes ids rather than an Item so a session's terminal side panel (W4) can render it too — that
 *  terminal has an item server-side, but it is hidden from every listing, so the client never holds one. */
...
/** The standalone terminal pane (a `terminal` item in the layout). */
export function TerminalPane({ item, visible }: PaneProps) {
  return <TerminalView terminalId={item.refId} title={item.title} visible={visible} />;
}
