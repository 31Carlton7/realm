...
export function parseQuestions(toolName: string, input: Record<string, unknown>): Question[] | null {
  ...
  for (const q of raw) {
    ...
    // A question with no options is a free-text prompt, which is legitimate — but only if free text
    // is actually offered. Both off together is a card with no row to answer on, and the session
    // would sit on it forever, so it falls back to the permission card like any other malformed input.
    if (options.length === 0 && allowOther === false) return null;
...
export function QuestionCard({ questions, onAnswer, onSkip, autoFocus = false, enter = false }: {
  ...
  const rowCount = q.options.length + Number(q.allowOther); // options, plus the free-text row when offered
      ...
      <div className="question-options">
        ...
        {q.allowOther && (othering ? (
          <div className="question-option question-other-edit">
            ...
            {/* A masked field keeps the value off the screen while it is typed. It cannot keep it out
                of the agent's own transcript — the agent asked for it and receives it as the answer. */}
            <input ref={otherInput} className="question-other-input" type={q.secret ? "password" : "text"} value={otherText}
              placeholder="Type your answer…" aria-label="Your answer" onChange={(e) => setOtherText(e.target.value)} />
