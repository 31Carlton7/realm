...
describe("the agent-controlled frame", () => {
  const frame = bodiesFor('.drive-frame[data-scope="pane"]').join(" ") + bodiesFor('.drive-frame[data-scope="window"]').join(" ");
  ...
  it("draws the ring at the shared weight", () => {
    ...
    expect(frame).toContain(`inset 0 0 0 ${AGENT_FRAME.ringPx}px var(--rl-accent)`);
  });
