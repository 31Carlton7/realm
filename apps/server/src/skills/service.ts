...
export class SkillsService {
  ...
  scanRoots(): string[] {
    return this.d.settings.getIds(SCAN_ROOTS_KEY);
  }
  ...
  list(spaceId: string): { root: string; skills: Skill[] } {
    ...
    const external = new Set(this.d.settings.getIds(externalEnabledKey(spaceId)));
  ...
  setEnabled(spaceId: string, id: string, enabled: boolean): void {
    ...
    if (isLibrary) {
      ...
      const on = new Set(this.d.settings.getIds(key));
