...
type Row = {
  ...
  endpoint_json: string | null; password_sealed: string | null; ws_port: number | null;
...
function toEndpoint(json: string | null): VncEndpoint | null {
  if (!json) return null;
  try {
    const v = JSON.parse(json) as { host?: unknown; port?: unknown };
...
export class MachinesStore {
  ...
  insert(input: { id: string; spaceId: string; name: string; source: MachineSource; endpoint: VncEndpoint | null; sealedPassword: string | null }): Machine {
    const t = now();
    this.db.prepare("INSERT INTO machines (id, space_id, name, source, endpoint_json, password_sealed, ws_port, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, NULL, ?, ?)")
      .run(input.id, input.spaceId, input.name, input.source, input.endpoint ? JSON.stringify(input.endpoint) : null, input.sealedPassword, t, t);
  ...
  update(id: string, patch: { name?: string; endpoint?: VncEndpoint | null; sealedPassword?: string | null }): Machine | null {
    ...
    this.db.prepare("UPDATE machines SET name = ?, endpoint_json = ?, password_sealed = ?, updated_at = ? WHERE id = ?")
      .run(patch.name ?? cur.name, endpoint ? JSON.stringify(endpoint) : null, sealed, now(), id);
    return this.get(id);
  }
