...
export type App = { port: number; db: Db; terminals: TerminalService; sessions: SessionService; browserAgents: BrowserAgentService; agentRuns: AgentRunService; reviews: ReviewService; runs: RunService; gateway: McpGateway; close(): Promise<void> };
...
export async function createApp(opts: { home: string; port: number; adapters?: AdapterRegistry; claudeDir?: string;
  ...
  const sessions = new SessionService({ db, rpc, sessions: sessionsStore, events: sessionEvents, items, spaces, projects, environments, settings, worktrees, ports, terminals, adapters: opts.adapters ?? defaultAdapters(), skills, gateway: mcpGateway, memory, checkpoints, browserPermissions: browserBroker,
    ...
    browserAgents: {
      ...
      release: (id) => { browserAgents?.release(id); agentRuns?.release(id); reviews?.release(id); forks?.release(id); runs?.release(id); },
  ...
  return {
    port, db, terminals, sessions, browserAgents, agentRuns, reviews, runs, gateway: mcpGateway,
    close: async () => {
