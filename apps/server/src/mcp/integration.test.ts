...
describe("mcp over rpc", () => {
  ...
  it("mcp.providers.list names the gateway's registered providers with THIS space's switch state (W4)", async () => {
    ...
    /* Every registered provider is listed with this space's switch state. All default ON except the
       ...
       declared further down `app.ts`. `goal` is on by default because its reach is the narrowest
       here (two tools that appear only on a session already pursuing a goal, and the most either can
       do is end it); `realm-schedule` because it only writes a row this space's own page can see. */
